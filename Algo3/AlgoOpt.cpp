...
bool AlgoOpt::chocDansCellule(PointOpt mvt, string ind)
{
    ...
    if(affichage)cout <<"ind : "<<ind<<", INDS : "<<INDS<<", INDP : "<<INDP<<", INDCHOC : "<<INDCHOC<<", listff.size() : "<<listff.size()<<endl;
...
int AlgoOpt::supprimerCroisement(PointOpt mtp, std::string ind){
    ...
    int indic = 0;
    PointOpt mt = listff.at(ind);
    if(affichage)cout << "ind : "<<ind<<" listff.size() "<<listff.size()<<endl;
    ...
    for (auto num:listNum){
        ...
        auto iterator = find(listeCellulesBrulees.begin(),listeCellulesBrulees.end(),num);
        if(iterator != listeCellulesBrulees.end()){
            if(affichage)cout << "num est present : "<<num<<endl;
...
pair<bool,vector<vector<VitesseOpt>>> AlgoOpt::vitesseNew1(vector<vector<VitesseOpt>> vits, Vector2D tauperp){
    ...
    if(affichage) cout << "Debut vitesseNew1, la liste des vitesses et le tau appartiennent au point qui est a propager" << endl;
...
bool AlgoOpt::testBordureBrulee(PointOpt point) {
    ...
    if(listeCoins.count(num) == 0) {
        ...
        if (dm.norm() > seuil) {
            ...
            if(affichage) cout << "listeSommets.count(num1) : "<< listeCoins.count(num1)  <<endl;
            if (listeCoins.count(num1) == 0) {//Ici le deuxieme sommet n'a pas encore brule
                ...
                if(affichage) cout <<"tmax : "<<tmax << ", tmax1 : "<<tmax1<<", tmin : "<<tmin << endl;
                if (tmax <= tmin && tmax1 <= tmin) {//Ici on test si l'insertion va se faire sur une Bordure brule
                    return true;
                }
