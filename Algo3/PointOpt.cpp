...
PointOpt::PointOpt(Point3D p, Vector2D tau,std::string indice, int * compt, std::string conditionInsertion):
tau(std::move(tau)),indice(std::move(indice)),coordonne(Point2D(p.x(),p.y())),t(p.z()), id(std::to_string(*compt)),creation(std::move(conditionInsertion)){
    ++(*compt);}
...
PointOpt::PointOpt(int * compt):
        tau(Vector2D()),indice("0"),coordonne(Point2D()),t(0), id (std::to_string(++(*compt))){++(*compt);}
