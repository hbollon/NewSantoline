...
def getQGISDirectory():
    ...
    qgisDirectory = str(programFiles).replace(
        '/', '\\') + "\\" + str(qgisLastVersion)
...
def createJsonPath():
    ...
    dataDirectory = str(currentDirectory).replace(
        '\SantolinePyQT5', '') + "\data"
    windMapDirectory = str(dataDirectory) + "\maps"
    asctojsonparserDirectory = str(currentDirectory).replace(
        '\SantolinePyQT5', '') + "\src\AscToJsonParser"
    ...
    windNinjaDirectory = str(windNinja).replace(
        '/', '\\') + "\\" + str(windNinjaLastVersion)
