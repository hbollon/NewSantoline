...
def adapt_grid_square_coords_to_canvas(factor_height, factor_width, height_canvas, new_list, width_canvas):
    ...
    if 'orig_cellule_coords' in convertedGrid[0]:
        ...
        for i in range(len(convertedGrid)):
            point = {"x": new_list[i]['orig_cellule_coords']
                     [0], "y": new_list[i]['orig_cellule_coords'][1]}
        ...
        for i in range(len(convertedGrid)):
            point = {"x": new_list[i]['cellule_coords']
                     [0], "y": new_list[i]['cellule_coords'][1]}
...
def adapt_initial_outline_to_canvas(factor_height, factor_width, height_canvas, new_list,
    ...
    for i in range(len(convertedInitial_outline["contourInitial"])):
        point = {"x": initial_outline["contourInitial"][i]
                 [0], "y": initial_outline["contourInitial"][i][1]}
        convertedInitial_outline["contourInitial"][i] = adapt_coords_to_canvas(
            factor, min_val, point, size_canvas)
