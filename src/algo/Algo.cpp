...
//######################################################################################################################
    contour.push_back(trier(finalContour));
//    contour.push_back(burn);
    return contour;
}
...
vector<Point3D> Algo::trier2(vector<Point3D> list){
    time_t debut;
    time(&debut);

    vector<Point3D> result;
    vector<Point3D> reste;
    double maxDist = 0;
    int marker = 0;
    Point3D p0 = *list.begin();
    result.push_back(p0);
    ...
    while(list.size()>1){
        ...
        double distanceTemp =distanceP(p0, p1);
        if (distanceTemp > 2) {
            if(distanceTemp<25){
                result.push_back(p1);
                p0=p1;
                ++i;
                if(distanceTemp>maxDist) {
    ...
    if(reste.size()>20){
        reste=trier2(reste);
        if(distanceP(*result.begin(),*result.end())>maxDist){
            result.insert(result.end(),reste.begin(),reste.end());
        }
        else{
            result.insert(result.begin()+marker,reste.begin(),reste.end());
        }
