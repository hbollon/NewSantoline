...
json Algo::algoMaillageVariable(){
    ...
    vector<ListePoint2D> listeObstacles = getObstacleFromJson(this->listeObstacles);
    ...
    cout << endl << "temps simulation = " << difftime(fin, debut) << " secondes" << endl;
...
vector<ListePoint2D> Algo::getObstacleFromJson(const json& listeSegment){

    vector<ListePoint2D> liste;

    for(const auto &it : listeSegment){

        ListePoint2D listeTemp;
...
ListeBurningPoint Algo::getBurningPointInitiaux(const ListePoint2D &pointsInitiaux, int dimention, int nbDivisionCellule){
    ...
    for (auto it = pointsInitiaux.begin(); it != pointsInitiaux.end(); ++it){    // On veut parcourir tous les segments du contour initial

        // création du segment
        Point2D p1 = *it;
        Point2D p2;

        if (it+1 == pointsInitiaux.end()){// si on est sur le dernier point
...
vector<vector<Point3D>> Algo::propagation(ListeBurningPoint& burningPointInitiaux, vector<ListePoint2D>& listeObstacles ){
    ...
    for (map<string, Point3D>::const_iterator it = burning.begin(); it != burning.end(); ++it){
        updateVicinity(&betterVicinity, burning,it->second,segment_size);
    }
    ...
    while (pointAMettreEnFeu.second.second.z() < duration)
    {
        ...
        if(intervalle>0){
            ...
            if(pointAMettreEnFeu.second.second.z() > durationInte  ){
                ...
                contour.push_back(contourCourant);
                vector<Point3D> contourCourant;
        ...
        if(!intersection) {
            burning[key] = a;
            updateVicinity(&betterVicinity,burning,a ,segment_size);
        }
    }
    //Enfin on ajoute le dernier contour

//    contour.push_back( get_contour(betterVicinity, listeObstacles,duration));
//    contour.push_back(contourTest);
...
vector<vector<Point3D>> Algo::propagationFixe(ListeBurningPoint& burningPointInitiaux, vector<ListePoint2D>& listeObstacles ){
    ...
    while (pointAMettreEnFeu.second.second.z() < duration)
    {
        ...
        if(intervalle>0){
            //Si la date d'ignition de "pointAMettreEnFeu" est superieur a "durationInte" alors il faut calculer un contour
            if(pointAMettreEnFeu.second.second.z() > durationInte  ){
                vector<Point3D> contourtemp = get_contour(betterVicinity,listeObstacles,durationInte);
                contourCourant.insert(contourCourant.end(),contourtemp.begin(), contourCourant.end());
                contour.push_back(contourCourant);
                vector<Point3D> contourCourant;
                durationInte += intervalle;
            }
...
//        test.push_back(Point3D( it->second.x(), it->second.y(), it->second.z()));
//    }
//    contour.push_back(test);
//
//
//    vector<Point3D> test2;
//    for (auto it = betterVicinity.begin(); it != betterVicinity.end(); ++it){
//        test2.push_back(Point3D( it->second.second.x(), it->second.second.y(), it->second.second.z()));
//    }
//
//
//    contour.push_back(test2);
...
vector<Point3D> Algo::trier(vector<Point3D> list){
    time_t debut;
    time(&debut);
    std::cout<<"Trie de "<<list.size()<<" elements ";
...
//            std::cout<<distanceP(*point,*it)<<std::endl;

            if (distanceP(p0, *it) < distanceP(p0, *temp) ) {
//                if(distanceP(p0, *it) < 1){
//                    list.erase(it);
//                }
//                else{
//                    temp = it;
...
//                }
        ...
        if (distanceP(p0, p1) > 2 && distanceP(p0,p1)<50 ){
            result.push_back(p1);
            p0=p1;
        }
    ...
    cout << endl << " en " << difftime(fin, debut) << " secondes" << endl;
    return result;
}
...
void Algo::updateVicinity(map<string,pair<Point3D,Point3D>> *vicinity, map<string, Point3D> burning,Point3D p,double segment_size){
...
// retourne tout les points touché par les arcs de l'ellipse(déterminée au point p) avec une date d'ignition = 200
{
    ...
    AEllipse a, b;
    if (fmod(p.x(), dimensionCellule) == 0){    // p est su un bord vertical de cellule
    ...
    else{   // p est sur un bord horizontal de cellule  (/!\ ici on suppose donc que p ne peux pas etre à l'intérieur d'une cellule)
        // on calcule les ellipse associées aux 2 cellules adjacentes
    ...
    vector<Point3D> result;
    for (vector<Point2D>::const_iterator it = arcs_a.begin(); it != arcs_a.end(); ++it){// on initialise la date d'ignition de points trouvés à 200 ->??
...
// retourne tout les points touché par les arcs de l'ellipse(déterminée au point p) avec une date d'ignition = 200
    ...
    Point2D test = getCentreFromPoint(p);
    arcs_centre(test, arcs);

    test = getCoinBasGaucheFromPoint(p);
    arcs_bas_gauche(test, arcs);

    test = getCoinBasDroitFromPoint(p);
    arcs_bas_droit(test, arcs);

    test = getCoinHautGaucheFromPoint(p);
    arcs_haut_gauche(test, arcs);

    test = getCoinHautDroitFromPoint(p);
    arcs_haut_droit(test, arcs);

    vector<Point3D> result;
    for (vector<Point2D>::const_iterator it = arcs.begin(); it != arcs.end(); ++it){// on initialise la date d'ignition de points trouvés à 200 ->??
...
void Algo::arcs_centre(const Point2D& p, vector<Point2D>& result){

    /*
    int nb_gen = 3;
    double gen = 25 * nb_gen;
    double dimension = 12.5;
    */
...
void Algo::arcs_bas_gauche(const Point2D& p, vector<Point2D>& result){

    int nb_gen = 1;
    double dimension = 12.5;
    double gen_negatif = -25 * (nb_gen + 1);
    double gen_positif = 25 * (nb_gen + 1);

    /* Coin to Coin */

    for(double x = gen_negatif ; x <= gen_positif ; x += dimension){
        for(double y = gen_negatif ; y <= gen_positif ; y += dimension){

            result.push_back(Point2D(p.x() + x, p.y() + y));
        }
    }
    ...
    /*
    gen_negatif = (-25 * nb_gen) + 12.5;
    gen_positif = (25 * nb_gen) - 12.5;
...
void Algo::arcs_bas_droit(const Point2D& p, vector<Point2D>& result){
...
void Algo::arcs_haut_gauche(const Point2D& p, vector<Point2D>& result){
...
void Algo::arcs_haut_droit(const Point2D& p, vector<Point2D>& result){
...
string Algo::associated_id(const Point2D& p, double segment_size, double cell_dimension)
{
    ...
    if (fmod(p.x(), cell_dimension) == 0) {
        result = to_string((int)p.x())
                 + '_'
                 + to_string(int(p.y() - fmod(p.y(), segment_size) + segment_size / 2.0));
    }
    else if (fmod(p.y(), cell_dimension) == 0) {
        result = to_string(int(p.x() - fmod(p.x(), segment_size) + segment_size / 2.0))
                 + '_'
                 + to_string((int)p.y());
    }
    else {
